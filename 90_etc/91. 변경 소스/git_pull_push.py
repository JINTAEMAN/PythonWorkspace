...
command = 'git log --oneline --all --graph -5'   # 커밋 로그 보기(최근 5개만 보여 주기)
proc = subprocess.Popen(command, shell=True, stdout=subprocess.PIPE).stdout
out_cmd = proc.read()
...
if str(git_way) == "push":   # 인자값이 push 이면(gitHub에 올리기)
    sMsg2 = "[T_60] [B. gitHub(원격 저장소)에 올리기 처리] ■■■■■ ★★ ■■■■■\n"
    ...
    if out_cmd_status_cd_1 < 0:  # 조회 단어1가 없으면
        ...
        #  --> ▶[24.11.06 09:50] 02. 소스 올리기(by 진태만[PC 명: TAMA-MSI-A10SD-I7-21Y])
