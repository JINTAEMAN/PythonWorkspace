...
- 41. 배포(VIOLET 화면에서[보안 Cloud PC]) @@@ ==> 02. Stg(검증계) ■■■
A. commai-ixi-api @@@
1. INSTANCE 생성
 1) Instance(화면) > CTEATE AN INSTANCE: 클릭
 2) New Instance(화면)
  가. Name: commai-Instance
  나. Image: (shared) 6: commai-server-base-2204-py31013-nginx ■  ==> Stg 서버
  다. Node Group: 
...
$ git checkout main		# dev Branch로 전환
...
2. SERVICE 생성
 ...
 2) Service Registry: ready(새창)
  ...
  라. Version: 10.0.7.6 --> 0.0.77	 ==> 신규 버전 생성 ■
  마. Release Note:
	241104
	- deploy 'dev' branch ==> TEST(by JTM)
==> SAVE: 클릭

 3) Service Manager(화면) > Service Manager Grid에서 Latest Version 칼럼에서 0.0.76(0.0.77)   ==> 데이타 확인 ■
  가. 성공: Created 칼럼에서: deployed --> ■
==============================================================================================================
 
3. SERVICE Deploy
 1) Service Manager(화면) > Service Manager Grid에서 Action 칼럼에서 0.0.76(0.0.77) ╊ 버튼(Deploy): 클릭
 2) Service Deploy(새창)
  가. Select Service Version: 0.0.77	 ==> 생성한 이미지 버전 ■
...
2. SERVICE 생성
 1) Instance(화면) > INSTANCE Grid에서 actions 칼럼 ▼ 버튼(Add Service): 클릭 ==> 서비스 추가
 2) Service Registry: ready(새창)
  가. Title: 6: commai-ixi-api(sc-commai-ixi-worker) ==> 기존 서비스: 선택 ■
...
3. SERVICE Deploy
 ...
 2) Service Deploy(새창)
  ...
  사. Contact Port: 8080  ==> APIKEY: Disable(체크  박스 해제) , Strict Affinity: Enable(체크 박스 설정) ■
  아. StartupProbe - Failure Threshold, AutoScale, AutoScale - Min Relicas, AutoScale - Max Relicas
	가) 300, 20,  20, 200	==> 검증 서버일 경우 ■
  자. Requests - CPU, Requests - Memory, Limits - CPU, Limits - Memory
	가) 1, 4Gi, 1, 4Gi	==> 검증 서버일 경우 ■
 차. Gpu
	0
  카. 노드 그룹
	가) Select Node Groups: eks-violet-service-communication-p4d-24xlarge-ng - Exist GPU	==> p4d: GPU 존재(LLM 일때 사용) ■
...
2. SERVICE 생성
 ...
 2) Service Registry: ready(새창)
  ...
  나. Service Type: Service Core: 선택(참조 --> Custom Reusable Service)
  다. Description: ==> TEST(by JTM)
  라. Version: 0.0.13 --> 0.0.14	 ==> 신규 버전 생성 ■
  마. Release Note:
	241104
	- deploy 'dev' branch ==> TEST(by JTM)
==> SAVE: 클릭

 3) Service Manager(화면) > Service Manager Grid에서 Latest Version 칼럼에서 0.0.13(0.0.14) ==> 데이타 확인 ■  
   가. 성공: Created 칼럼에서: deployed --> ■
==============================================================================================================

3. SERVICE Deploy
 1) Service Manager(화면) > Service Manager Grid에서 atest Version 칼럼에서 0.0.13(0.0.14) ==> 데이타 확인 ■
 ==> Action 칼럼 ╊ 버튼(Deploy): 클릭 
 2) Service Deploy(새창)
  가. Select Service Version: 0.0.14	 ==> 생성한 이미지 버전 ■
  나. Select Rentime Language: Python
  다. Command:./bin/bash
  ...
  사. Contact Port: 8080  ==> APIKEY: Disable(체크  박스 해제) , Strict Affinity: Enable(체크 박스 설정) ■
  아. StartupProbe - Failure Threshold, AutoScale, AutoScale - Min Relicas, AutoScale - Max Relicas
	가) 300, 20, 1, 1		==> 검증 서버일 경우 ■
  자. Requests - CPU, Requests - Memory, Limits - CPU, Limits - Memory
	가) 500m, 500Mi, 500m, 500Mi	==> 검증 서버일 경우 ■
 차. Gpu
	0
  카. 노드 그룹   
	가) Select Node Groups: eks-violet-service-communication-p4d-24xlarge-ng - Exist GPU	==> p4d: GPU 존재(LLM 일때 사용) ■
...
https://sc-commai-ixi-api.stg-sp.violet.uplus.co.kr/api/v1/text
# 02. 프로젝트 키(Project Key): 검증 서버
...
https://sc-commai-ixi-api.stg-sp.violet.uplus.co.kr/api/v1/text/job-status

$ /app/client/text/get_text_result.sh		# step 3. post tex 결과 확인 ♣
 https://sc-commai-ixi-api.stg-sp.violet.uplus.co.kr/api/v1/text/result

$ /app/client/tts/tts.sh		# step 4. post tts 실행 ♣ 
https://sc-commai-ixi-api.stg-sp.violet.uplus.co.kr/api/v1/audio/tts
=============================================================================================================
■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■
+--------------------------------------------------------// End //--------------------------------------------------+ 
■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■ 
