 # ! /pythonWorkspace_pu.py   # 메모 파일 복사 파일 
...
def file_copy(src, dst)： 	# 파일 복사 처리 함수()
	try : 
		print("\n[@_T] ■■ [/pythonWorkspace_pu.py] [file_copy()] ==> ■■■■■■ [T_01 [ob_srcsort]"+ str(src) +"[dstr】"+ str(dst) )
		
		shutil.copy2(src, dst)   # 파일 복사
		print("\n[@_T] ■■ [/ob.sqljm.copy.py] [file.copy()] ==> [T_11] [파일 복사 성공]")

		# if not os.path.isfilefdst)+    # dst 파일이 없다면 ==> dst 경로 인식 못 함
		#  print("\n[@_T] ■■ [/pythonWorkspace_pu.py] file_copy()] ==> ■■■■■■ [T_02] [src】"+ str(src) +"[dst】"+  str(dst)) 

		#  if not os.path.isdir(dst)     # 복사할 곳에 디액토리가 없다연
		#       print("\n[@_T] ■■ [/pythonWorkspace_pu py] file_copy()] ==> ■■■■■■ [T_03] [src】"+  str(src) +"[dst]"+ str(dst))

		# dst_dir = os.path dirname(dst)    # 디렉토리 경로 계산
		#  # os.makedirs(dst_dir)    # 디액토리 생성 

		# shutil.copy2(src, dst)    # 파일 복사
		#         pnnt(An[[@_T] ■■ [/pythonWorkspace_pu.py] [file_copy()] ==>■■■■■■ [T_11】    [파일 복사 성공】_) 

		# else :
		# pnnt("[@_T] ■■ [/pythonWorkspace_pu.py] [file_copy()] ==>■■■■■■ [T_11】    [파일 복사 성공】")

	except Exception as e:
		print(e)
		print("\n[@_T] ■■ [/pythonWorkspace_pu.py] [file_copy()] ==> [T_91] [파일 복사 실패] [Error]"+ e )
...
print("\n[@_T] ■ [/pythonWorkspace_pu.py] ==> [T_51_0] [table_list] [i_번째]"+ i +"[tb_nmp]"+ str(tb_nm) +"[src_path]"+ str(src_path) +"[dst_path]"+ str(dst_path) )

file_copy(src_pathr, dst_path)   # 파일 복사 처리(Git 로컬 저장소에 파일 저장) ■■■■■

csv_res = list()   # csv_res 설정 
print("\n\n\n\n[@_TT] ■■■ [/pythonWorkspace_pu.py] ==> [T_01] ■■■■■■ [######################### [OB sql 파일 복사 쳐리 Start] #########################] ■■■■■■ ")
...
print("\n[@_T] ■■ [/pythonWorkspace_pu py] ==> [T_63_3] [5. 원격 方!장소에 반영(git push)]"+ out_cmd_push.decode('utf-8'))
print("\n\n [@_T] ■■■ [/ob_sqlJm.copy py] ==> [T_80 [Git 정보 쳐리 시작 End] ■■■ @@@@@ ■■■■■■■■■ ■\n\n\n\n\n\n")


print("\n\n[@_TT] ■■■ [/pythonWorkspace_pu.py] ==> [T_99] ■■■■■■ [######################### [CSV 파일 정보 확인 End] #########################] ■■■■■■\n\n\n\n")
