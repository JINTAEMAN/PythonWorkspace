...
path = './table_list.txt'           # 파일 경로
res_path = './file_info.csv'      # 파일 명
f = open(path, 'r') 
...
for i in table_list: 
    ...
    with open(res_path, 'w') as csv_file:	    # data 디렉토리안에 res_path 경로의 파일을 생성	
    # // write = csv.writer(file)    # 쓰고 싶은 내용 입력
    csv_file.writer(result)    # 쓰고 싶은 내용 입력
