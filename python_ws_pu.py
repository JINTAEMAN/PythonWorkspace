...
if ob_sort > 0 :
    ...
    if ob_sort_nm == "get":   # 인자값이 get 이면
        print("[@_T] ■■■ [/python_ws_pu.py] ==> [T_60_2] [Git 경보 처리 시작0. 원격 저장소 데이다 가저 오기(git pull)] ■■■■■■■■■■ \n\n")

    command = 'git pull origin main'    # 0. 원격 저장소 데이다 가저 오기
    proc = subprocess.Popen(command, shell=True, stdout=subprocess.PIPE).stdout
    out_cmd = proc.read()
    print("[@_T] ■■■ [/python_ws_pu.py] ==> [T_61] [0. 원격 저장소 데이다 가저 오기(git pull)]"+ out_cmd.decode('utf-8') ) 

    command = 'git log --oneline --all --graph'   # 로그 보기
    proc = subprocess.Popen(command, shell=True, stdout=subprocess.PIPE).stdout
    out_cmd = proc.read()
    print("[@_T] ■■■ [/python_ws_pu.py] ==> [T_62] [0. 로그 보기]"+ out_cmd.decode('utf-8') ) 
